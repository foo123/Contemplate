...
function localized_date (locale, format, timestamp) {
    // localized version of phpjs date function
    var that = this,
      jsdate,
      f,
      formatChr = /\\?([a-z])/gi,
  ...
  f = {
    ...
    D: function () { // Shorthand day name; Mon...Sun
      var w=txt_words[f.w()];
      return (locale[w]) ? locale[w] : w;
    },
    j: function () { // Day of month; 1..31
      return jsdate.getDate();
    },
    l: function () { // Full day name; Monday...Sunday
      var w=txt_words[f.w()] + 'day';
      return (locale[w]) ? locale[w] : w;
    },
    N: function () { // ISO-8601 day of week; 1[Mon]..7[Sun]
      return f.w() || 7;
    ...
    S: function () { // Ordinal suffix for day of month; st, nd, rd, th
      var j = f.j();
      var w=(['st', 'nd', 'rd'][j % 10 - 1] || 'th');
      var l= (locale[w]) ? locale[w] : w;
      return j < 4 | j > 20 && l;
    },
    w: function () { // Day of week; 0[Sun]..6[Sat]
      return jsdate.getDay();
    ...
    M: function () { // Shorthand month name; Jan...Dec
      var w=txt_words[6 + f.n()].slice(0, 3);
      return (locale[w]) ? locale[w] : w;
    },
    n: function () { // Month; 1...12
      return jsdate.getMonth() + 1;
    ...
    // Time
    a: function () { // am or pm
      var lam=(locale['am']) ? locale['am'] : 'am', lpm=(locale['pm']) ? locale['pm'] : 'pm';
      return jsdate.getHours() > 11 ? lpm : lam;
    },
    A: function () { // AM or PM
      return f.a().toUpperCase();
    },
