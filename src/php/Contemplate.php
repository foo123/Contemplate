...
class Contemplate
{
    ...
    {
        return is_array($o) ? array_keys($o) : array();
    }

    public static function values($o)
    {
        return is_array($o) ? array_values($o) : array();
    }
    ...
    {
        return count(is_array($o) ? $o : (array)$o);
    }
    ...
    {
        return preg_replace('#</?[a-zA-Z0-9:_\\-]+[^<>]*>#sum', '', $s);
    }
    ...
    protected static function parse_constructs($match)
    {
        ...
        switch ($ctrl)
        {
            case 's': $out = 'strval(' . $args . ')'; break;
            case 'n': $out = 'intval(' . $args . ')'; break;
            ...
            case 'tpl':
                ...
                // no break
            default:
                if (is_callable(array(__CLASS__, $ctrl)))
                {
    ...
    protected static function parse($tpl, $leftTplSep, $rightTplSep, $withblocks = true)
    {
        ...
        while ($tpl && strlen($tpl))
        {
            ...
            $isphp = 'php:' === substr($tpl, $p1+$l1, 4);
            // js literal code block
            $isjs = 'js:' === substr($tpl, $p1+$l1, 3);
            // py literal code block
            $ispy = 'py:' === substr($tpl, $p1+$l1, 3);
            if ($isphp || $isjs || $ispy)
            {
                // include if in same language else ignore
                if ($isphp)
                {
                    if ('=' === substr($tpl, $p1+$l1+4, 1))
                    {
                        $parsed .= "';" . self::align("\n/* php code start */") . self::align("\n\$__p__ .= (string)(" . trim(substr($tpl, $p1+$l1+5, $p2-($p1+$l1+5))) . ");") . self::align("\n/* php code end */\n\$__p__ .= '");
                    }
                    else
                    {
                        $indent = 0; $l3 = 0; $indenttype = 'none';
                        if (preg_match('#^(postdent|predent)\\((-?\\d+)\\):#', substr($tpl, $p1+$l1+4), $m))
                        {
                            $indenttype = $m[1];
                            $indent = intval($m[2]);
                            $l3 = strlen($m[0]);
                        }
                        $code = self::remove_blank_lines(substr($tpl, $p1+$l1+4+$l3, $p2-($p1+$l1+4+$l3)));
                        if ('predent' === $indenttype)
                        {
                            self::$__level = max(0, self::$__level + $indent);
                        }
                        $parsed .= "';" . self::align("\n/* php code start */");
                        if (strlen(trim($code)))
                        {
    ...
    protected static function align($s, $level = null)
    {
        ...
        if ($l && (0 < $level))
        {
            ...
            for ($i=0; $i<$l; ++$i)
            {
                ...
                elseif ($is_line_start)
                {
                    // consistently replace tabs with our tabbed spaces
                    if (preg_match(self::$SPACE, $c))
                    {
                        $aligned .= "\t" === $c ? self::$__pad : $c;
                    }
                    else
                    {
                        $aligned .= $c;
    ...
    protected static function remove_initial_space($s)
    {
        $l = strlen($s);
        if ($l)
        {
            ...
            if ($sl)
            {
                ...
                while (false !== $pos)
                {
                    if ($initial_space === substr($s, $pos+1, $sl))
                    {
                        $s = substr($s, 0, $pos+1) . substr($s, $pos+1+$sl);
                    }
                    $pos = strpos($s, "\n", $pos + 1);
                }
            }
    ...
    protected static function remove_blank_lines($s)
    {
        $lines = explode("\n", $s);
        $n = count($lines);
        $start = 0; $end = $n-1;
        ...
        return implode("\n", array_slice($lines, $start, max(0, $end-$start+1)));
    }
}
